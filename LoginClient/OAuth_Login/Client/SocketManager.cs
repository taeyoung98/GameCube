...
namespace SocketClient
{
    ...
    class SocketManager
    {
        ...
        private void StartListenHost()
        {
            while (true)
            {
                try
                {
                    ...
                    GettedMessageFromHost(Encoding.UTF8.GetString(data), HostClient);
        ...
        private void HandleClient(int id, TcpClient c)
        {
            while (true)
            {
                try
                {
                    ...
                    GettedMessageFromClient(Encoding.UTF8.GetString(data), id, c);
                }
                catch
                {
        ...
        {
            byte[] toBytes = Encoding.UTF8.GetBytes(str);
            return toBytes;
        }
